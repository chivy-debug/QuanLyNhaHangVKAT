...
namespace QuanLyNhaHangVKAT.DAO
{
    public class AccountProvider
    {
        ...
        public bool login(string userName, string passWord)
        {
            ...
            DataTable loGin = DataProvider.Instance.ExecuteQuery(query, new object[] {userName , hashedPassword});
        ...
        public bool UpdateAccountInformation(string userName , string displayName , string passWord , string newPass) // trong fAccountInformation
        {
            ...
            int result = DataProvider.Instance.ExecuteNonQuery(" EXEC USP_UpdateAccount @username , @displayname , @password , @newpassword ",new object[] {userName , displayName , hashedPassword , hashedNewPassword});
            return result > 0;
        }
        ...
        {
            string hashedPassword = CalculateMD5Hash(password);
            string query = string.Format("INSERT dbo.Account ( UserName, DisplayName, Type, password )VALUES  ( N'{0}', N'{1}', {2}, N'{3}')", name, displayName, type, hashedPassword);
        ...
        {
            string hashedPassword = CalculateMD5Hash(password);
            string query = string.Format("UPDATE dbo.Account SET DisplayName = N'{1}', Type = {2} , Password = N'{3}' WHERE UserName = N'{0}'", name, displayName, type , hashedPassword);
            int result = DataProvider.Instance.ExecuteNonQuery(query);
